    ...
    return (
      <a href={`/#/product/${product.id}`}>
        <div className="font-Inter" {...rest}> <img className="bg-white" src='k-swiss.png' alt="Product Image" />  
         ...
         <br /> 
         <span className="text-[#1F1F1F] font-normal mr-3 text-base md:text-2xl line-through opacity-30">${parseInt(product.price)}</span>
         <span className="text-[#1F1F1F] font-bold text-base md:text-2xl">${(product.price / 2).toFixed()}</span>
        </div>
      </a>
    )
  }